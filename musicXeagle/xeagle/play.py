...
def cb_admin_check(func: Callable) -> Callable:
    async def decorator(client, cb):
        ...
        if cb.from_user.id in admemes:
            ...
            await cb.answer("You have no rights!", show_alert=True)
            return
...
@Client.on_message(
    filters.command("music") & ~filters.edited & ~filters.bot & ~filters.private
)
@authorized_users_only
async def hfmm(_, message):
    ...
    if len(message.command) != 2:
        await message.reply_text(
            "I only recognize `/music on` and /music `off only`"
        )
        return
...
async def play(_, message: Message):
    ...
    if audio:
        ...
        title = file_name
        thumb_name = "https://telegra.ph/file/5dd3d6d64ccd785ae5af2.jpg"
